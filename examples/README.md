...
| `grand.py` | The piano bank. An electric grand (GM 2, a CP-70) against the acoustic one, and `--honky` for GM 3 at three wheel positions. `--register` is the test: the same figure low, where the short case makes its bass up to twelve times stiffer, and two octaves up, where both pianos use a string of the same length and only the missing soundboard is left. |
