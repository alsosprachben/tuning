...
| `portamento.py` | The three portamento controllers, and the argument that a glide is a LENGTH moving rather than a pitch. Reproduces Roland's own two worked CC84 examples byte for byte -- the only cases specified by a primary source -- and measures what falls out of `f = 1/L`: the same interval takes the same time in either direction but not the same path, and four times longer two octaves down. `--check` measures without rendering. |
